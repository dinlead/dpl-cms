 ...
 */
(function eventStatus(Drupal, once) {
  Drupal.behaviors.event_status = {
    attach(context) {
      function ribbonMarkup(label, color, xPos, yPos) {
        xPos = xPos || 'right';
        yPos = yPos || 'top';
        return `<div class="ribbon ribbon-${yPos}-${
          xPos
        }"><span style="background-color: ${color}">${label}</span></div>`;
      }

      /* Find elements with ribbon data attributes (works regardless of class) */
      once('event-status', '[data-ribbon-label]', context).forEach(
        function processRibbon(display) {
